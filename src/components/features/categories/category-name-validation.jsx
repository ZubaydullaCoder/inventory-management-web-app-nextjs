// /src/components/features/categories/category-name-validation.jsx
"use client";

import { CheckCircle, XCircle, Loader2 } from "lucide-react";

/**
 * Category name validation indicator component
 * @param {Object} props
 * @param {string} props.watchedName - Current name being watched
 * @param {Object} props.category - Original category data
 ...
 */
export default function CategoryNameValidation({
  watchedName,
  category,
  isChecking,
  isUnique,
  ...
  if (category && watchedName === category.name) return null; // No need to validate unchanged name
  ...
  if (hasChecked && !isUnique) {
    return (
      <div className="flex items-center text-sm text-red-600">
        <XCircle className="w-4 h-4 mr-1" />A category with this name already
        exists
      </div>
    );
  }
